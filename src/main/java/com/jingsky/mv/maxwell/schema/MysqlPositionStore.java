...
public class MysqlPositionStore {
	...
	public void set(Position newPosition) throws SQLException, DuplicateProcessException {
		...
		String sql = "INSERT INTO `"+ TablePrefixConfig.getTablePrefix()+"well_positions` set "
				+ "server_id = ?, "
				+ "gtid_set = ?, "
				+ "binlog_file = ?, "
				+ "binlog_position = ?, "
	...
	private Long insertHeartbeat(Connection c, Long thisHeartbeat) throws SQLException, DuplicateProcessException {
		String heartbeatInsert = "insert into `"+ TablePrefixConfig.getTablePrefix()+"well_heartbeats` set `heartbeat` = ?, `server_id` = ?, `client_id` = ?";
	...
	private void heartbeat(Connection c, long thisHeartbeat) throws SQLException, DuplicateProcessException {
		if ( lastHeartbeat == null ) {
			PreparedStatement s = c.prepareStatement("SELECT `heartbeat` from `"+ TablePrefixConfig.getTablePrefix()+"well_heartbeats` where server_id = ? and client_id = ?");
			s.setLong(1, serverID);
		...
		String heartbeatUpdate = "update `"+ TablePrefixConfig.getTablePrefix()+"well_heartbeats` set `heartbeat` = ? where `server_id` = ? and `client_id` = ? and `heartbeat` = ?";
	...
	public Position getLatestFromAnyClient() throws SQLException {
		try ( Connection c = connectionPool.getConnection() ) {
			PreparedStatement s = c.prepareStatement("SELECT * from `"+ TablePrefixConfig.getTablePrefix()+"well_positions` where server_id = ? ORDER BY last_heartbeat_read desc limit 1");
			s.setLong(1, serverID);
	...
	public Position get() throws SQLException {
		try ( Connection c = connectionPool.getConnection() ) {
			PreparedStatement s = c.prepareStatement("SELECT * from `"+ TablePrefixConfig.getTablePrefix()+"well_positions` where server_id = ? and client_id = ?");
			s.setLong(1, serverID);
			s.setString(2, clientID);
	...
	protected List<RecoveryInfo> getAllRecoveryInfos(Connection c) throws SQLException {
		PreparedStatement s = c.prepareStatement("SELECT * from `"+ TablePrefixConfig.getTablePrefix()+"well_positions` where client_id = ? order by last_heartbeat_read DESC");
	...
	protected List<String> formatRecoveryFailure(MaxwellConfig config, List<RecoveryInfo> recoveries) {
		if (recoveries.size() == 0) {
			return Collections.singletonList("Unable to find any binlog positions in `"+ TablePrefixConfig.getTablePrefix()+"well_positions` table");
		}

		ArrayList<String> result = new ArrayList<>();
		Long mostRecentMaster = recoveries.get(0).serverID;

		result.add("Found multiple binlog positions for cluster in `"+ TablePrefixConfig.getTablePrefix()+"well_positions` table.  Not attempting position recovery.");
		...
		if (allRecoveryInfos.size() > 1) {
			...
			try (Connection c = connectionPool.getConnection()) {
				PreparedStatement s = c.prepareStatement(
					"DELETE FROM `"+ TablePrefixConfig.getTablePrefix()+"well_positions` WHERE server_id <> ? AND client_id = ?"
				);
				s.setLong(1, serverID);
				s.setString(2, clientID);
				s.execute();
