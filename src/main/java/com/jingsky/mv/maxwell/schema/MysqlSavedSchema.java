...
public class MysqlSavedSchema {
	...
	private final static String columnInsertSQL =
		"INSERT INTO `"+ TablePrefixConfig.getTablePrefix()+"well_columns` (schema_id, table_id, name, charset, coltype, is_signed, enum_values, column_length) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
	...
	private Long findSchemaForPositionSHA(Connection c, String sha) throws SQLException {
		PreparedStatement p = c.prepareStatement("SELECT * from `"+ TablePrefixConfig.getTablePrefix()+"well_schemas` where position_sha = ?");
		p.setString(1, sha);
	...
	private Long saveDerivedSchema(Connection conn) throws SQLException {
		PreparedStatement insert = conn.prepareStatement(
				"INSERT into `"+ TablePrefixConfig.getTablePrefix()+"well_schemas` SET base_schema_id = ?, deltas = ?, binlog_file = ?, " +
	...
	public Long saveSchema(Connection conn) throws SQLException {
		...
		schemaInsert = conn.prepareStatement(
				"INSERT INTO `"+ TablePrefixConfig.getTablePrefix()+"well_schemas` SET binlog_file = ?, binlog_position = ?, server_id = ?, charset = ?, version = ?, position_sha = ?, gtid_set = ?, last_heartbeat_read = ?",
				Statement.RETURN_GENERATED_KEYS
		);

		databaseInsert = conn.prepareStatement(
				"INSERT INTO `"+ TablePrefixConfig.getTablePrefix()+"well_databases` SET schema_id = ?, name = ?, charset=?",
				Statement.RETURN_GENERATED_KEYS
		);

		tableInsert = conn.prepareStatement(
				"INSERT INTO `"+ TablePrefixConfig.getTablePrefix()+"well_tables` SET schema_id = ?, database_id = ?, name = ?, charset=?, pk=?",
				Statement.RETURN_GENERATED_KEYS
		);
	...
	private HashMap<Long, HashMap<String, Object>> buildSchemaMap(Connection conn) throws SQLException {
		...
		PreparedStatement p = conn.prepareStatement("SELECT * from `"+ TablePrefixConfig.getTablePrefix()+"well_schemas`");
		ResultSet rs = p.executeQuery();
	...
	private void restoreSchemaMetadata(Connection conn, Long schemaID) throws SQLException {
		PreparedStatement p = conn.prepareStatement("select * from `"+ TablePrefixConfig.getTablePrefix()+"well_schemas` where id = " + schemaID);
	...
	private void restoreFullSchema(Connection conn, Long schemaID) throws SQLException, InvalidSchemaError {
		PreparedStatement p = conn.prepareStatement(
				"SELECT " +
						...
						"FROM `"+ TablePrefixConfig.getTablePrefix()+"well_databases` d " +
						"LEFT JOIN "+ TablePrefixConfig.getTablePrefix()+"well_tables t ON d.id = t.database_id " +
						"LEFT JOIN "+ TablePrefixConfig.getTablePrefix()+"well_columns c ON c.table_id=t.id " +
						"WHERE d.schema_id = ? " +
	...
	private static Long findSchema(Connection connection, Position targetPosition, Long serverID)
		...
		if (targetBinlogPosition.getGtidSetStr() != null) {
			PreparedStatement s = connection.prepareStatement(
				"SELECT id, gtid_set from `"+ TablePrefixConfig.getTablePrefix()+"well_schemas` "
				+ "WHERE deleted = 0 "
				+ "ORDER BY id desc");
			...
			PreparedStatement s = connection.prepareStatement(
				"SELECT id from `"+ TablePrefixConfig.getTablePrefix()+"well_schemas` "
				+ "WHERE deleted = 0 "
	...
	public static void delete(Connection connection, long schema_id) throws SQLException {
		connection.createStatement().execute("update `"+ TablePrefixConfig.getTablePrefix()+"well_schemas` set deleted = 1 where id = " + schema_id);
	}
